...
- 提交与上传文案约束（owner 于 2026-09-12 再次确认，持续生效）：每次执行前必须核对根目录 [AGENTS.md](../AGENTS.md) 的“提交与小程序上传文案（硬规则）”，该节为唯一维护位置；覆盖 Git 标题、正文和小程序上传实际填写或自动生成的全部说明。
...
7. 小程序上传与验收分开：仅修改小程序、无需部署后端时，按 owner 已明确的偏好，相关验证通过后直接上传开发版本，无需先 commit/push；微信后台的版本管理、提审及发布仍由 owner 操作，不自动扩大授权。具体顺序和命令以 `miniapp/README.md` 的“开发版本上传操作规范”为准。交付必须分别说明本地验证、CLI 上传回执、后台版本确认和真机症状验收；未观察到的阶段不得宣称完成。尤其源码属性断言、类型检查和构建通过不能证明键盘、惯性滚动、图片裁切等真机体验已修好；owner 确认运行新版本后仍复现时，回到症状与布局/运行时证据，不继续归咎缓存或只改版本号。
