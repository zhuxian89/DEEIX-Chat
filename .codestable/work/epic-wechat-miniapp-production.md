...
- 2026-09-03：ITEM-3 体验补强。小程序按成熟 Web 的流事件契约接入 `process_update` 与 `upstream_think_delta`，将模型思考和 MCP 工具轨迹分开保存、实时展示并支持折叠；只有工具轨迹真实出现 `web_search_exa` / `web_fetch_exa` 时才显示联网搜索状态，同时覆盖历史消息与断线续传，不改后端。版本更新为 `0.1.20.20260903`；`pnpm check` 通过（TypeScript、88 项测试、微信生产构建、隔离与产物安全检查）。
- 2026-09-03：owner 纠正正式小程序 AppID 为 `wx59fcdf6143e32cef`，此前测试号永久废弃。正式 AppID 已写入可跟踪的 `project.config.json`、模板、配置脚本及上传前检查；版本 `0.1.22.20260903` 重新构建后，微信 CLI 明确以正式 AppID 返回 `√ upload`。是否完成以上传版本出现在正式小程序后台“开发版本”为最终判据。
- 2026-09-03：新增每日签到幸运转盘。独立 `daily_checkin_claims` 以 `(user_id, business_date)` 唯一约束兜底，签到记录、原有美元余额增量和 `daily_checkin_reward` 流水同事务提交；并发/重试返回首次中奖快照。后台参数配置提供原子 JSON 的开关、`0.00167 USD/次`、`Asia/Shanghai` 时区和次数/基点概率编辑器；小程序 `0.1.25.20260903` 提供转盘、到账金额、冷启动/回前台恢复和余额刷新。`pnpm api:check`、定向 Go 测试、`go vet ./...`、后台 Biome 检查及小程序 `pnpm check`（最终新增测试后 95 tests + 微信生产构建 + 产物检查）通过。
